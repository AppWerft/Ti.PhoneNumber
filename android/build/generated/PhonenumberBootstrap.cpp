...
#define TAG "ti.phoneneumber"

using namespace v8;

static Persistent<Object> bindingCache;

static void Phonenumber_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Phonenumber.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::PhonenumberBindings::lookupGeneratedInit(
...
static void Phonenumber_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Phonenumber_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Phonenumber_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::PhonenumberBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry PhonenumberBinding = {
	"ti.phoneneumber",
	Phonenumber_init,
	Phonenumber_dispose
};
...
Java_ti_phoneneumber_PhonenumberBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.phoneneumber", &PhonenumberBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::PhonenumberBindings::lookupGeneratedInit));
}
